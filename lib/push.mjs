...
// Какие уведомления присылать. Ключи хранятся отдельно от подписок
export const PREF_KINDS = [
  { id: "turn", title: "Твой ход", sub: "в играх" },
  { id: "result", title: "Итог партии", sub: "кто победил" },
  { id: "notes", title: "Заметки", sub: "когда второй пишет" },
  { id: "flame", title: "Огонёк", sub: "напомнить поджечь" },
];
const DEFAULT_PREFS = { turn: true, result: true, notes: true, flame: true };

export async function getPrefs(userId) {
  const p = await store().get("prefs:" + userId, { type: "json" });
...
export async function setPrefs(userId, prefs) {
  ...
  for (const k of Object.keys(DEFAULT_PREFS)) {
    if (prefs && prefs[k] !== undefined) next[k] = !!prefs[k];
  }
  await store().setJSON("prefs:" + userId, next);
  return next;
}
...
// kind - тип уведомления, его можно выключить в настройках. Без kind шлём всегда.
export async function send(userId, payload, kind) {
  if (!ready()) return { skipped: "no-keys" };
  if (kind) {
    const prefs = await getPrefs(userId);
    if (!prefs[kind]) return { skipped: "off" };
  }
  const list = (await store().get(userId, { type: "json" })) || [];
